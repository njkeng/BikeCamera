...
function reset_default_configuration() {
    ...
    user_reset_files=$(sudo cat /etc/pihelmetcam/hostapd/reset.ini | grep --only-matching --perl-regexp "(?<=user_reset_files = ).")
    ...
    if [ "$user_reset_files" == "0" ]; then
		...
		sudo rm /etc/pihelmetcam/pihelmetcam.auth
	    sudo cp $webroot_dir/config/user_pihelmetcam.auth /etc/pihelmetcam/pihelmetcam.auth
	fi
}

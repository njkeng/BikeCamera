...
function reset_default_configuration() {
	...
	webroot_dir=$(sudo cat /etc/bikecamera/hostapd/reset.ini | grep --only-matching --perl-regexp "(?<=webroot_dir = \")\S+(?=\")")

    	echo Restoring BikeCamera defaults
	    ...
	    sudo rm /etc/bikecamera/bikecamera.auth
